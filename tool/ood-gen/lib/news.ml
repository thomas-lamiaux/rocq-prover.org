...
module RssFeed = struct
  let feed_authors authors =
    ...
    | x :: xs -> (x, xs)
    | [] -> (Syndic.Atom.author "Rocq Prover News", [])

  let create_entry (post : t) =
    let content = Syndic.Atom.Html (None, post.body_html) in
    let id = Uri.of_string ("https://rocq-prover.org/news/" ^ post.slug) in
  ...
  let create_feed () =
    ...
    |> entries_to_feed ~id:"news.xml" ~title:"Rocq Prover News @ rocq-prover.org"
    |> feed_to_string
end
